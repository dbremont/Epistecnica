...
src/note/README.md); violations print as warnings and never fail the
build.
...
def main() -> int:
    ...
    print(f"notes-index: {len(notes)} notes ({tagged} tagged), {sum(n['words'] for n in notes)} words -> {OUT.relative_to(NOTE.parent.parent)} ({kb:.0f} KB)")
