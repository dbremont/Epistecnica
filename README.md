# Epistecnica

> Epistecnica is a unified framework for the two fundamental dimensions of human agency: knowing (Epistemica) and acting (Tecnica).

> Epistemica describes how an agent knows; Tecnica describes how an agent acts. Epistecnica brings both under a common index, deployment, and set of conventions, without collapsing their conceptual distinction or their independence as subprojects.

## References

- [Philosophia Artium Technicarum et Operis](https://www.notion.so/Philosophia-Artium-Technicarum-et-Operis-355c0f5171ec808b82f8d7a85e8134cd?source=copy_link)
- [Modelling Modelling](https://www.notion.so/Framework-334c0f5171ec803e8cfbe7f0bc02c575?source=copy_link)
- [Affirmation Space](https://www.notion.so/Affirmation-Space-336c0f5171ec80928f75ddbde09d7121?source=copy_link)
- [Algorithms](https://github.com/dbremont/algorithms)
- [autoregia](https://github.com/dbremont/autoregia)
